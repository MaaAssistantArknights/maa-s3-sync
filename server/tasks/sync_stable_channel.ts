...
export default defineTask({
  ...
  async run({ payload }) {
      ...
      transport: {
        targets: [
          ...
          },
          {
            target: 'pino-pretty',
            level: 'info',
            options: { destination: `./logs/task_sync_stable_channel/${uuid}.log`, colorize: false, translateTime: 'SYS:standard' },
          },
        ]
      }
    })
    ...
    try {
      ...
      logger.info('Step 4. Filter the latest stable version')
      ...
      logger.info('Step 5. Get download URLs of stable version packages')
